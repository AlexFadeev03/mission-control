...
interface ClaudeCodeDoctorFixProgress {
  step: string
  detail: string
}
...
export function ClaudeCodeDoctorBanner() {
  const t = useTranslations('doctorBanner')
  const tc = useTranslations('common')
  const [doctor, setDoctor] = useState<ClaudeCodeDoctorStatus | null>(null)
  ...
  async function loadDoctorStatus() {
    try {
      const res = await fetch('/api/claude-code/doctor', { cache: 'no-store' })
      if (!res.ok) {
        setDoctor(null)
        return
      }
  ...
  async function handleFix() {
    ...
    try {
      const res = await fetch('/api/claude-code/doctor', { method: 'POST' })
      const data = await res.json()
      ...
      setDoctor(data.status)
      const progress = Array.isArray(data.progress) ? data.progress as ClaudeCodeDoctorFixProgress[] : []
