 ...
 * Agent Config Sync
 *
 * Reads agents from claude-code.json and upserts them into the MC database.
...
function getConfigPath(): string | null {
  return config.claudeCodeConfigPath || null
}

function resolveAgentWorkspacePath(workspace: string): string {
  if (isAbsolute(workspace)) return resolve(workspace)
  if (!config.claudeCodeStateDir) {
    throw new Error('CLAUDE_CODE_STATE_DIR not configured')
  }
  return resolveWithin(config.claudeCodeStateDir, workspace)
}
...
/** Read and parse claude-code.json agents list */
async function readClaudeCodeAgents(): Promise<ClaudeCodeAgent[]> {
  const configPath = getConfigPath()
  if (!configPath) throw new Error('CLAUDE_CODE_CONFIG_PATH not configured')
...
/** Extract MC-friendly fields from a Claude Code agent config */
function mapAgentToMC(agent: ClaudeCodeAgent): {
  name: string
  role: string
  config: any
...
/** Preview the diff between claude-code.json and MC database without writing */
...
/** Write an agent config back to claude-code.json agents.list */
export async function writeAgentToConfig(agentConfig: any): Promise<void> {
  ...
  const normalizedAgentConfig = normalizeAgentConfigForClaudeCode(agentConfig)

  // Find existing by id
  const idx = parsed.agents.list.findIndex((a: any) => a.id === normalizedAgentConfig.id)
  if (idx >= 0) {
    // Deep merge: preserve fields not in update
    parsed.agents.list[idx] = normalizeAgentConfigForClaudeCode(
...
function normalizeAgentConfigForClaudeCode(agentConfig: any): any {
