 ...
 * 3. Fallback: `gateway.tailscale.mode === 'serve'` in claude-code.json (legacy)
 */
export function detectTailscaleServe(web: Record<string, any> | null | undefined, configPath?: string): boolean {
  ...
  // Legacy: check claude-code.json config
  const effectivePath = configPath || process.env.CLAUDE_CODE_CONFIG_PATH || ''
