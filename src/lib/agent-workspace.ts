...
function resolvePath(candidate: string): string {
  ...
  if (!config.claudeCodeStateDir) throw new Error('CLAUDE_CODE_STATE_DIR not configured')
  return resolveWithin(config.claudeCodeStateDir, candidate)
}
...
export function getAgentWorkspaceCandidates(agentConfig: any, agentName: string): string[] {
  ...
  const claudeCodeIdRaw =
    typeof agentConfig?.claudeCodeId === 'string' && agentConfig.claudeCodeId.trim()
      ? agentConfig.claudeCodeId.trim()
      : agentName
  const claudeCodeId = claudeCodeIdRaw.toLowerCase().replace(/[^a-z0-9._-]+/g, '-')

  push(rawWorkspace || null)
  push(`workspace-${claudeCodeId}`)
  push(`agents/${claudeCodeId}`)
  push('workspace')
