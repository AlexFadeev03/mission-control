...
async function readClaudeCodeEnvFile(envFilePath: string): Promise<Map<string, string>> {
  try {
...
export async function getEffectiveEnvValue(
  ...
  const envFilePath = options?.envFilePath || join(config.claudeCodeStateDir, '.env')
  const envMap = await readClaudeCodeEnvFile(envFilePath)
