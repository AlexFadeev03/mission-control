...
import { archiveOrphanTranscriptsForStateDir } from '@/lib/claude-code-doctor-fix'
...
function isMissingClaudeCode(detail: string): boolean {
...
export async function GET(request: Request) {
  ...
  try {
    const result = await runClaudeCode(['doctor'], { timeoutMs: 15000 })
    return NextResponse.json(parseClaudeCodeDoctorOutput(`${result.stdout}\n${result.stderr}`, result.code ?? 0, {
      stateDir: config.claudeCodeStateDir,
    }), {
      headers: { 'Cache-Control': 'no-store' },
    })
  } catch (error) {
    const { detail, code } = getCommandDetail(error)
    if (isMissingClaudeCode(detail)) {
      return NextResponse.json({ error: 'Claude Code is not installed or not reachable' }, { status: 400 })
    }

    return NextResponse.json(parseClaudeCodeDoctorOutput(detail, code ?? 1, {
      stateDir: config.claudeCodeStateDir,
    }), {
      headers: { 'Cache-Control': 'no-store' },
    })
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const fixResult = await runClaudeCode(['doctor', '--fix'], { timeoutMs: 120000 })
    progress.push({ step: 'doctor', detail: 'Applied Claude Code doctor config fixes.' })

    try {
      await runClaudeCode(['sessions', 'cleanup', '--all-agents', '--enforce', '--fix-missing'], { timeoutMs: 120000 })
    ...
    const orphanFix = archiveOrphanTranscriptsForStateDir(config.claudeCodeStateDir)
    progress.push({
      step: 'orphans',
      detail:
    ...
    const postFix = await runClaudeCode(['doctor'], { timeoutMs: 15000 })
    const status = parseClaudeCodeDoctorOutput(`${postFix.stdout}\n${postFix.stderr}`, postFix.code ?? 0, {
      stateDir: config.claudeCodeStateDir,
    })
    ...
    logger.error({ err: error }, 'Claude Code doctor fix failed')

    return NextResponse.json(
      {
        error: 'Claude Code doctor fix failed',
        detail,
        status: parseClaudeCodeDoctorOutput(detail, code ?? 1, {
          stateDir: config.claudeCodeStateDir,
        }),
      },
      { status: 500 }
