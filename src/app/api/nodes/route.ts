...
export async function GET(request: NextRequest) {
  ...
  if (action === 'list') {
    try {
      ...
      try {
        const data = await callClaudeCodeGateway<{ nodes?: unknown[] }>('node.list', {}, GATEWAY_TIMEOUT)
  ...
  if (action === 'devices') {
    try {
      ...
      try {
        const data = await callClaudeCodeGateway<{ devices?: unknown[] }>(
          'device.pair.list',
          {},
          GATEWAY_TIMEOUT,
        )
...
export async function POST(request: NextRequest) {
  ...
  try {
    const result = await callClaudeCodeGateway(spec.method, params, GATEWAY_TIMEOUT)
